...
export function render(game, el, activeTab) {
    const mode = MODES[game.runMode],
        target = TARGETS[game.targetTier];
    ...
    el.heatStatePill.textContent = heatState(game.heat);
...
export function renderTabContent(game, mount, activeTab) {
    ...
    for (const item of items) {
        ...
        const owned = !!ownedMap[item.id],
            ready = canBuy(game, activeTab, item),
            cc = activeTab === 'prestige' ? 0 : runtimeCredits(game, item),
            dc = activeTab === 'prestige' ? 0 : runtimeData(game, activeTab, item),
            nodeLocked = activeTab === 'nodes' && !game.nodesUnlocked ? 'Research Node Theory first' : activeTab === 'nodes' && Object.keys(game.nodesOwned).length >= game.nodeSlots && !owned ? `Node slots full (${game.nodeSlots})` : '';
        const div = document.createElement('div');
        div.className = 'system-card';
        div.innerHTML = `<div class='system-top'><div><div class='system-name'>${item.name}</div><div class='system-desc'>${item.desc}</div></div><div class='pill'>${owned?'Owned':'Ready'}</div></div><div class='meta'><span>${activeTab==='prestige'?`${item.costFragments} Fragments`:`${cc?fmt(cc)+' Credits':''}${cc&&dc?' · ':''}${dc?fmt(dc)+' Data':''}`}</span><span>${item.bandwidth?`${item.bandwidth} BW`:activeTab==='nodes'?'Node':'No BW cost'}</span></div>${item.requires?`<div class='small'>Requires: ${item.requires.join(', ')}</div>`:''}${nodeLocked?`<div class='small' style='color:var(--red)'>${nodeLocked}</div>`:''}<button class='buy-btn ${ready?'ready':''}' data-buy='${activeTab}:${item.id}' ${owned||!ready?'disabled':''}>${owned?'Installed':ready?'Buy':nodeLocked||'Unavailable'}</button>`;
